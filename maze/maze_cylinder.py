"""Maze Cylinder — Yantra4D Hyperobject Cartridge (CadQuery / B-Rep).

A maze wrapped around a cylindrical shell; the field wraps in x, so it closes on itself with no east or west rim.
...
def build(params):
    ...
    grid = mz_grid(rows, cols, seed, x_wrapping=True)
    walls = mz_walls(grid, rows, cols, cell_size, x_wrapping=True)
